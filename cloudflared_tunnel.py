        ...
        try:
            ...
            if tunnel_id or config_path:
                ...
                tunnel_url = re.search(
                    "(?P<url>https?:\/\/[^\s]+.trycloudflare.com)", metrics
                ).group("url")
                break
        except:
            time.sleep(3)
...
def _start_cloudflared(port, metrics_port, tunnel_id=None, config_path=None):
...
def run(
    ...
    tunnel_port = tunnel_port or random.randint(8100, 9000)
    metrics_port = metrics_port or random.randint(9100, 9900)
    _start_cloudflared(tunnel_port, metrics_port, tunnel_id, config_path)
    yield tunnel_port
